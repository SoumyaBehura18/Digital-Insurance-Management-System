<template>
  ...
  <div class="bg-white rounded-lg border border-gray-200 p-6">
    ...
    <div v-else-if="$store.state.error" class="text-center py-12">
      ...
      <button @click="loadAdminClaims" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
        Try Again
      </button>
    </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="claim in filteredClaims" :key="claim.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4">
                ...
                >
                  <svg class="w-1.5 h-1.5 mr-1.5 rounded-full" :class="{
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click="closeModal">
      <div class="bg-white rounded-lg p-6 max-w-2xl w-full mx-4" @click.stop>
        <div class="flex justify-between items-start mb-6">
          <h3 class="text-xl font-bold text-gray-900">Claim Details - #{{ selectedClaim?.id }}</h3>
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-gray-900">${{ formatAmount(selectedClaim?.claimAmount) }}</p>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Date Submitted</label>
            <p class="text-gray-900">{{ formatDate(selectedClaim?.claimDate) }}</p>
          </div>
        </div>
        ...
        <div class="mb-6">
          ...
          <span 
            ...
            :class="{
              'bg-yellow-100 text-yellow-800': selectedClaim?.status === 'PENDING',
              'bg-green-100 text-green-800': selectedClaim?.status === 'APPROVED',
          ...
          >
            ...
            {{ selectedClaim?.status === 'PENDING' ? 'Pending' : selectedClaim?.status }}
          </span>
        </div>

        <!-- Admin Response Section (for pending claims) -->
        <div v-if="selectedClaim?.status === 'PENDING'" class="mb-6">
          <label class="block text-sm font-medium text-gray-700 mb-2">Admin Response (Required)</label>
          <textarea 
        ...
        <div v-else-if="selectedClaim?.reviewerComment" class="mb-4">
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            ...
            <p class="text-xs text-gray-500 mt-2">
              Reviewed on {{ formatDate(selectedClaim?.resolvedDate) }} by {{ selectedClaim?.reviewerName || 'Admin' }}
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async updateClaimStatus(claim, newStatus) {
      if (!claim.tempComment?.trim()) {
        alert('Please add a review comment before proceeding.');
        return;
      }
  ...
  await this.$store.dispatch(actionName, {
        ...
        alert(`Claim #${claim.id} has been ${action} successfully!`);
