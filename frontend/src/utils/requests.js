...
export async function makeRequestWithToken(type, endpoint, body) {
    const url = base_url + endpoint;
        ...
        const token = parsedUser?.token || localStorage.getItem('authToken') || localStorage.getItem('token');
    ...
    try {
        ...
        if (status === 401 || status === 403) {
            ...
            if (typeof window !== 'undefined') {
                ...
                const loginUrl = '/login' + (current && current !== '/login' ? `?redirect=${encodeURIComponent(current)}` : '');
