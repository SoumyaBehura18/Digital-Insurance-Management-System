          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                {{ userCache[ticket.userId]?.name || "Loading..." }}
              </td>
...
const getUserName = (customerId) => {
  const user = userCache.value[customerId];
...
const fetchAllTickets = async () => {
  try {
    ...
    const response = await store.dispatch("adminTickets/fetchAllTickets");
    tickets.value = Array.isArray(response) ? response : response.tickets;

    // 2. Get unique customer IDs
    const uniqueCustomerIds = [...new Set(tickets.value.map((t) => t.userId))];
    ...
    if (uniqueCustomerIds.length > 0) {
      try {
        ...
        const newUserCache = { ...userCache.value };
        users.forEach((user) => {
          newUserCache[user.id] = user;
        });

        // Update the reactive reference with the new object
        userCache.value = newUserCache;
      } catch (error) {
        // Fallback with the same reactivity fix
        ...
        results.forEach((result) => {
          if (result.status === "fulfilled" && result.value) {
            newUserCache[result.value.customerId] = result.value.user;
          }
        });
        userCache.value = newUserCache;
      }
